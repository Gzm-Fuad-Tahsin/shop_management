...
const getSaleDistribution = (sale) => {
  ...
  if (explicitCash > 0 || explicitBank > 0) {
    return { cash: explicitCash, bank: explicitBank }
  }

  const method = String(sale.paymentMethod || "").toLowerCase()
  if (["cash", "check"].includes(method)) {
    return { cash: Number(sale.totalAmount || 0), bank: 0 }
  }
...
router.get("/today", verifyToken, async (req, res) => {
  try {
    ...
    const previousCashAgg = await PreviousCash.aggregate([
      { $match: { shop, date: { $gte: start, $lte: end } } },
...
router.post("/today/previous-cash", verifyToken, async (req, res) => {
  try {
    ...
    const entry = await PreviousCash.create({
      shop,
